...
@app.route('/validarEM',methods=['GET','POST'])
def validarConclusaoEM():
	txt = request.get_data()
	obj = json.loads(txt)
	documento = obj['cpf']

	select = "select nivel from registro where cpf = '"+documento+"' and nivel = 'EM';"
...
@app.route('/validarES',methods=['GET','POST'])
def validarConclusaoES():
	txt = request.get_data()
	obj = json.loads(txt)
	documento = obj['cpf']

	select = "select nivel from registro where cpf = '"+documento+"' and nivel = 'ES' ;"
	conexao = sqlite3.connect('banco.data')
	resultado = conexao.execute(select).fetchall()
	conexao.close()
	if len(resultado)>0:
